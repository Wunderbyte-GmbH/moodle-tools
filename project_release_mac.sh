...
project_lowercase=$(printf '%s' "$project" | tr '[:upper:]' '[:lower:]')
...
create_allinone_branch() {
    ...
    # Remove everything except the repository metadata, including hidden files.
    clean_worktree_except_git
...
calculate_release_tag() {
    ...
    if [ -z "$existing_tags" ]; then
        ...
        patch_version=$(printf '%s\n' "$latest_tag" | awk -F. '{print $NF}')
...
# Remove every item in the current directory except .git. Using Bash globbing
# here avoids GNU-only find options and includes hidden files on macOS.
clean_worktree_except_git() {
    local item
...
prompt_and_change_directory() {
    read -r -p "Enter the directory path to execute the commands (default: $execute_directory): " user_input
...
read -r -p "Continue executing the script? (y/n): " continue_execution
...
read -r -p "Enter the new tag, leave empty for default. (default: $calculated_tag): " releasetag
...
# Clean the working tree while preserving .git. This also removes hidden files
# and avoids temporarily placing .git in a possibly occupied parent directory.
echo "Cleaning working tree while preserving .git"
clean_worktree_except_git
